...
const modelIcons: LucideIcon[] = [Smartphone, Wifi, Gauge, Server];
const deploymentIcons: LucideIcon[] = [Cpu, Server, Cloud];
const formFactorIcons: LucideIcon[] = [HardDrive, Database, Boxes, Layers3];
const featureIcons: LucideIcon[] = [ShieldCheck, PlugZap, Network, RadioTower, Route];
const suiteIcons: LucideIcon[] = [Router, Cloud, Network, Gauge];
const resourceIcons: LucideIcon[] = [Building2, Database, Globe2];

export const metadata: Metadata = {
  title: "CloudPort Edge | SD-WAN Edge India",
  description:
    "Deploy Lavelle CloudPort Edge for zero-touch SD-WAN branch networking across physical, virtual, and cloud locations.",
  alternates: {
    canonical: "/products/cloudport-edge",
  },
  openGraph: {
    title: "CloudPort Edge | Lavelle Networks",
    description:
      "Branch SD-WAN edge appliances for link independence, zero-touch provisioning, segmentation, internet protection, and path selection.",
    url: "/products/cloudport-edge",
    images: [
      {
        url: "/og/default.png",
        width: 1200,
        height: 630,
        alt: "CloudPort Edge by Lavelle Networks",
      },
    ],
  },
};
...
const productJsonLd = {
  ...
  url: `${SITE_URL}/products/cloudport-edge`,
};

function getEdgeContent() {
  if (!edge) {
    throw new Error("CloudPort Edge content is missing.");
  }

  return edge;
}

export default function CloudPortEdgePage() {
  const content = getEdgeContent();
    ...
    <div className="bg-white">
      ...
      <section className="relative isolate overflow-hidden bg-[#003F72] px-4 py-28 text-white sm:px-6 sm:py-32 xl:flex xl:min-h-[100svh] xl:items-center xl:px-8">
        ...
        <div className="mx-auto grid w-full max-w-7xl items-center gap-8 xl:grid-cols-[minmax(0,0.65fr)_minmax(0,1.35fr)] xl:gap-10">
          ...
          <div className="mt-12 sm:mt-16 xl:mt-0 w-full rounded-xl border border-white/15 bg-white/10 p-2 sm:p-3 shadow-2xl shadow-[#1A1A1A]/30 backdrop-blur xl:-mr-8 2xl:-mr-0">
            <div className="rounded-lg bg-[#F7FAFD] p-4 text-[#1A1A1A] sm:p-5">
              ...
              <div className="grid gap-4 py-4 xl:grid-cols-[1fr_0.8fr]">
                <div className="rounded-lg border border-[#C8D6E5]/60 bg-white p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-sm font-semibold uppercase text-[#005A9E]">
                        {content.applianceTitle}
                      </p>
                      <p className="mt-3 text-sm leading-6 text-[#4A4A4A]">
                        {content.applianceDescription}
                      </p>
                    </div>
                ...
                <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
                  {content.applianceStats.map((stat) => (
                    <div
                      key={stat.name}
                    ...
                    >
                      ...
                      <p className="mt-2 text-sm leading-6 text-[#4A4A4A]">{stat.description}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {product.models.map((model, index) => {
              const Icon = modelIcons[index] ?? Router;
      ...
      <section className="bg-white px-4 py-20 sm:px-6 sm:py-24 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-4xl text-center">
            <h2 className="font-display text-3xl text-[#1A1A1A] sm:text-4xl">
              {content.deploymentTitle}
            </h2>
            <p className="mt-5 text-lg leading-8 text-[#4A4A4A]">
              {content.deploymentDescription}
            </p>
          </div>

          <div className="mt-14 grid gap-6 md:grid-cols-3">
            {content.deploymentOptions.map((option, index) => {
              const Icon = deploymentIcons[index] ?? Cloud;

              return (
                <article key={option.name} className="rounded-xl border border-[#C8D6E5]/60 bg-[#F7FAFD] p-7">
                  <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-lg bg-[#003F72] text-white">
                    <Icon className="h-6 w-6" aria-hidden="true" />
                  </div>
                  <h3 className="font-display text-xl text-[#1A1A1A]">{option.name}</h3>
                  <p className="mt-4 text-sm leading-6 text-[#4A4A4A]">{option.description}</p>
                </article>
              );
            })}
          </div>
          ...
          <div className="mt-14">
            <h3 className="text-center font-display text-2xl text-[#1A1A1A]">
              {content.formFactorsTitle}
            </h3>
            <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
              {content.formFactors.map((factor, index) => {
                const Icon = formFactorIcons[index] ?? HardDrive;

                return (
                  <article
                    key={factor.name}
                    className="rounded-xl border border-[#C8D6E5]/60 bg-white p-6 text-center shadow-sm"
                  >
                    <div className="mx-auto mb-5 flex h-12 w-12 items-center justify-center rounded-lg bg-[#E6F3FB] text-[#0078D4]">
                      <Icon className="h-6 w-6" aria-hidden="true" />
                    </div>
                    <h4 className="font-display text-lg text-[#1A1A1A]">{factor.name}</h4>
                    <p className="mt-2 text-sm text-[#4A4A4A]">{factor.description}</p>
                  </article>
                );
              })}
            </div>
            ...
            <div className="grid gap-5">
              {content.capabilities.map((capability, index) => {
                ...
                return (
                  <article
                    key={capability.name}
                    className="rounded-xl border border-[#C8D6E5]/60 bg-white p-6 shadow-sm sm:p-8"
                  >
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {content.suiteItems.map((item, index) => {
              ...
              return (
                <article
                  key={item.name}
                  className="rounded-xl border border-[#C8D6E5]/60 bg-[#F7FAFD] p-6 shadow-sm transition duration-200 hover:scale-[1.02] hover:shadow-lg"
                >
      ...
      <section className="bg-[#F7FAFD] px-4 py-20 sm:px-6 sm:py-24 lg:px-8">
        <div className="mx-auto max-w-5xl rounded-xl border border-[#C8D6E5]/60 bg-white p-8 text-center shadow-sm sm:p-12">
          <h2 className="font-display text-3xl text-[#1A1A1A] sm:text-4xl">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-3">
            {content.resources.map((resource, index) => {
              ...
              return (
                <article
                  key={resource.title}
                  className="rounded-xl border border-[#C8D6E5]/60 bg-[#F7FAFD] p-7 shadow-sm transition duration-200 hover:scale-[1.02] hover:shadow-lg"
                >
