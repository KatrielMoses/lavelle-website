...
const modelIcons: LucideIcon[] = [Server, Gauge, Boxes, Router];
const featureIcons: LucideIcon[] = [Route, Split, Share2, Layers3, Shuffle, Cloud];
const suiteIcons: LucideIcon[] = [RadioTower, Cloud, Network, Gauge];
const resourceIcons: LucideIcon[] = [Building2, Database, Globe2];

export const metadata: Metadata = {
  title: "CloudPort Gateway | SD-WAN Gateway India",
  description:
    "Aggregate Lavelle SD-WAN edge traffic with CloudPort Gateway for BGP, OSPF, DC/DR, cloud, and high-capacity data center connectivity.",
  alternates: {
    canonical: "/products/cloudport-gateway",
  },
  openGraph: {
    title: "CloudPort Gateway | Lavelle Networks",
    description:
      "High-capacity SD-WAN gateway aggregation for data centers, cloud networks, BGP, OSPF, NFV, and DC/DR.",
    url: "/products/cloudport-gateway",
    images: [
      {
        url: "/og/default.png",
        width: 1200,
        height: 630,
        alt: "CloudPort Gateway by Lavelle Networks",
      },
    ],
  },
};
...
const productJsonLd = {
  ...
  url: `${SITE_URL}/products/cloudport-gateway`,
};

function getGatewayContent() {
  if (!gateway) {
    throw new Error("CloudPort Gateway content is missing.");
  }

  return gateway;
}

export default function CloudPortGatewayPage() {
  const content = getGatewayContent();
        ...
        <div className="mx-auto grid w-full max-w-7xl items-start gap-8 xl:grid-cols-[minmax(0,0.65fr)_minmax(0,1.35fr)] xl:gap-10">
          ...
          <div className="mt-12 sm:mt-16 xl:mt-0 w-full rounded-xl border border-white/15 bg-white/10 p-2 sm:p-3 shadow-2xl shadow-[#1A1A1A]/30 backdrop-blur xl:-mr-8 2xl:-mr-0">
            <div className="rounded-lg bg-[#F7FAFD] p-4 text-[#1A1A1A] sm:p-5">
              ...
              <div className="grid gap-4 py-4 xl:grid-cols-[1fr_0.86fr]">
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {product.models.map((model, index) => {
              const Icon = modelIcons[index] ?? Server;
