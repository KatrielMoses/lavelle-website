    ...
    <main className="pb-24">
      ...
      <section className="bg-white py-16 px-4 sm:px-6 lg:px-8 border-b border-gray-100">
        <div className="mx-auto max-w-7xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
            ...
            <div>
              <h2 className="text-3xl font-bold tracking-tight text-[#1A1A1A] font-cormorant mb-6">
                Challenges in {industry.title}
              </h2>
              <ul className="space-y-4">
                {industry.challenges.map((challenge, idx) => (
                  <li key={idx} className="flex gap-4">
                    <span className="flex-shrink-0 mt-1 flex h-6 w-6 items-center justify-center rounded-full bg-[#D4AF37]/10 text-[#D4AF37]">
      ...
      <section className="bg-white pt-16 pb-8 px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          <h2 className="text-3xl font-bold tracking-tight text-center text-[#1A1A1A] font-cormorant mb-12">
            Benefits of Lavelle Networks' SD-WAN
          </h2>
          <div className="space-y-6">
            {industry.benefits.map((benefit, idx) => (
              <div key={idx} className="bg-gray-50 border-l-4 border-[#D4AF37] p-6 rounded-r-lg">
                <p className="text-lg text-[#4A4A4A]">{benefit}</p>
              </div>
            ))}
          </div>
        </div>
