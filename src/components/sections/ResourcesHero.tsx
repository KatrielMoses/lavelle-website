...
import { RESOURCES_HERO } from "@/lib/resources-content";
  ...
  return (
    <section className="relative isolate flex min-h-[80svh] items-center overflow-hidden bg-[#021530] px-4 py-28 text-white sm:px-6 sm:py-32 xl:px-8">
      ...
      >
        ...
        <motion.p
          variants={shouldReduce ? undefined : itemVariants}
          className="mx-auto mt-8 max-w-3xl font-display text-xl text-[#1A9AE6] sm:text-2xl lg:text-3xl"
        >
          {RESOURCES_HERO.command}
        </motion.p>
        ...
        >
          <Link
            href={RESOURCES_HERO.primaryCta.href}
          ...
          >
            {RESOURCES_HERO.primaryCta.label}
            <ArrowRight className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-0.5" aria-hidden="true" />
          </Link>
          <Link
            href={RESOURCES_HERO.secondaryCta.href}
          ...
          >
            {RESOURCES_HERO.secondaryCta.label}
          </Link>
        </motion.div>
      </motion.div>
    </section>
